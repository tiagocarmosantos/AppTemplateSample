...
(function () {
		...
		function picMeAPI(config) {
				...
				var _getPicsMe = function _getPicsMe(filter) {
						var url = !!filter ? config.oapiUrl.getOApiUrl() + "/picMe/?filter=" + filter.param + ":" + filter.value : config.oapiUrl.getOApiUrl() + "/picMe";
				...
				var _getPicMe = function _getPicMe(id) {
						return fetch(config.oapiUrl.getOApiUrl() + "/picMe/" + id).then(function (response) {
								return response.json();
						}).then(function (data) {
								return data;
						}).catch(function (error) {
				...
				var _deletePicMe = function _deletePicMe(id) {
						return fetch(config.oapiUrl.getOApiUrl() + "/picMe/" + id, { method: 'DELETE' }).then(function (response) {
								return response.json();
						}).then(function (data) {
								return data;
						}).catch(function (error) {
...
(function () {
	...
	function quickMenuAPI($http, config) {
		...
		var _getQuickMenus = function _getQuickMenus() {
			return $http.get(config.oapiUrl.getOApiUrl() + "/quickMenu").then(function (response) {
		...
		var _getQuickMenu = function _getQuickMenu(id) {
			return $http.get(config.oapiUrl.getOApiUrl() + "/quickMenu/" + id).then(function (response) {
				return JSON.parse(JSON.stringify(response.data));
			}).catch(function (error) {
				throw Error('Aconteceu um problema: Não foi possível carregar os dados!');
			});
		...
		var _saveQuickMenu = function _saveQuickMenu(quickMenu) {
			return $http.post(config.oapiUrl.getOApiUrl() + "/quickMenu", quickMenu).then(function (response) {
				return JSON.parse(JSON.stringify(response.data));
			}).catch(function (error) {
		...
		var _deleteQuickMenu = function _deleteQuickMenu(id) {
			return $http.delete(config.oapiUrl.getOApiUrl() + "/quickMenu/" + id).then(function (response) {
				return JSON.parse(JSON.stringify(response.data));
			}).catch(function (error) {
...
(function () {
	...
	angular.module("ListaTelefonica").constant("config", {
		...
		appInitials: "NgP",
		appHome: { url: "", getAppHome: function getAppHome() {
		...
		apiUrl: { url: "https://microservice-sample.herokuapp.com/api", apiPort: "3003", getApiUrl: function getApiUrl() {
				return location.hostname.toLowerCase() == 'localhost' ? "http://localhost:" + this.apiPort + "/api" : this.url;
			} },
		oapiUrl: { url: "https://microservice-sample.herokuapp.com/oapi", apiPort: "3003", getOApiUrl: function getOApiUrl() {
				return location.hostname.toLowerCase() == 'localhost' ? "http://localhost:" + this.apiPort + "/oapi" : this.url;
			} },
